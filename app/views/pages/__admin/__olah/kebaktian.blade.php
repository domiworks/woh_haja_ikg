...
<script>
	...
	$('body').on('click', '#f_search_kebaktian', function(){		
		...
		$.ajax({
			type: 'POST',
			url: "{{URL('admin/search_kebaktian')}}",
			data : {
				'json_data' : json_data
				// 'data' : $data
			},
	...
	$('body').on('click', '.visibleButton', function(){
		...
		$.ajax({
			type: 'POST',
			url: "{{URL('admin/change_visible_kebaktian')}}",
			data: {
				'json_data' : json_data
			},
			success: function(response){							
...
@include('pages.__admin.__olah.popup_edit_kebaktian')
@include('pages.__admin.__olah.popup_delete_warning_kebaktian')
