...
<script>
	...
	$('body').on('change','#excel_import_kebaktian',function(){
		...
		$.ajax({
			...
			url: "{{URL('admin/import_kegiatan')}}/"+{{Session::get('id_gereja')}},
			data: $formData,
			processData:false,
			contentType:false,
	...
	$('body').on('click', '#f_eksport_kebaktian', function(){
		...
		//window.open("{{URL('admin/export_kegiatan')}}/"+{{Session::get('id_gereja')}}+"/"+$tahun_pelayanan,'_blank');		
		window.open("{{URL('admin/export_kegiatan')}}/"+{{Session::get('id_gereja')}}+"/"+tahun_awal+"/"+tahun_akhir,'_blank');
	...
	$('body').on('change','#excel_import_anggota',function(){
		...
		$.ajax({
			type: 'POST',
			// url: "{{--URL('user/import_kegiatan')--}}/"+{{--$header['id_gereja']--}},
			url: "{{URL('admin/import_anggota')}}/"+{{Session::get('id_gereja')}},
			data: $formData,
			processData:false,
			contentType:false,
			success: function(response){
	...
	$('body').on('click', '#f_eksport_anggota', function(){
		...
		var tahun_awal = arr_tahun_pelayanan[0];
		var tahun_akhir = arr_tahun_pelayanan[1];		
		window.open("{{URL('admin/export_anggota')}}/"+{{Session::get('id_gereja')}}+"/"+tahun_awal+"/"+tahun_akhir,'_blank');
	});
</script>
