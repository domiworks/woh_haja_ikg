...
<script>
	...
	$('body').on('click', '.visibleButton', function(){
		...
		$.ajax({
			type: 'POST',
			url: "{{URL('superadmin/change_visible_jenis_kegiatan')}}",
			data: {
				'json_data' : json_data
			},
	...
	$('body').on('click', '#f_post_jenis_kebaktian', function(){
		...
		$.ajax({
			type: 'POST',
			url: "{{URL('superadmin/post_jenis_kegiatan')}}",
			data : {
				'json_data' : json_data
			},
			success: function(response){
...
@include('pages.__superadmin.popup_edit_jenis_kegiatan')
@include('pages.__superadmin.popup_delete_warning_jenis_kegiatan')
